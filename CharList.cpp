...
void CharList::push(char c)
{
    ...
    this->end->next = temp;                // Get last node to point to it
    this->end = temp;                      // Make it the new end
} 
...
char CharList::dequeue()
{
    ...
    CharNode *temp;                     // Point to the node after start
    temp = start->next;

    delete start;                       // delete the start node
